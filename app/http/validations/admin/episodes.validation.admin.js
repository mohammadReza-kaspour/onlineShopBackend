...
const updateEpisodeValidation = () => [
    param("episodeid").isMongoId().withMessage("شناسه محصول وارد شده مجاز نمیباشد"),
    body("title").trim().isLength({min:0 , max:50}).withMessage("عنوان اپیزود باید بین 0 تا 50 کارکتر باشد"),
    body("text").trim().isLength({min:0}).withMessage("توضیحات اپیزود نمی تواند خالی باشد"),
    body("type").custom((value , {req}) => {
        if(!!value){
            const validData = ["free" , "cash" , "vip"];
            if(!validData.includes(value)) throw "نوع اپیزود باید یکی از مقادیر رایگان،پولی یاویژه باشد"
